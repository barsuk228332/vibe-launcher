...
class WineLauncher(ctk.CTk):
    def __init__(self):
        ...
        self.config_file = "my_games.json"
        ...
        # 🔥 АВТОСОЗДАНИЕ ЯРЛЫКА В МЕНЮ ПРИЛОЖЕНИЙ
    ...
    def create_desktop_shortcut(self):
        """Создает .desktop файл в ~/.local/share/applications/ для интеграции в меню KDE"""
...
"""
            try:
                ...
                print("🔥 Ярлык в меню приложений успешно создан!")
    ...
    def init_main_screen(self):
        ...
        self.top_box.pack(fill="x", pady=(0, 15))

        self.add_btn = ctk.CTkButton(self.top_box, text="+ Добавить игру", command=self.open_add_new)
        self.add_btn.pack(side="left", expand=True, fill="x", padx=(0, 5))

        self.settings_btn = ctk.CTkButton(self.top_box, text="⚙️ Настройки", width=100, fg_color="#444", hover_color="#555", command=lambda: self.show_screen(self.settings_screen))
        self.settings_btn.pack(side="right", padx=(5, 0))

        self.search_entry = ctk.CTkEntry(self.main_screen, placeholder_text="Поиск игры...")
        self.search_entry.pack(pady=(0, 10), fill="x")
        self.search_entry.bind("<KeyRelease>", self.filter_games)

        self.scroll_frame = ctk.CTkScrollableFrame(self.main_screen, label_text="Твоя библиотека игр", label_font=("Arial", 12, "bold"))
        self.scroll_frame.pack(fill="both", expand=True, pady=5)

        self.info_panel = ctk.CTkFrame(self.main_screen, fg_color="#2b2b2b", height=135)
        self.info_panel.pack(fill="x", pady=10)
        self.info_panel.pack_propagate(False)
        
        self.info_label = ctk.CTkLabel(self.info_panel, text="Выберите игру из списка", justify="left", text_color="gray", anchor="w")
        self.info_label.pack(fill="both", padx=15, pady=10)

        self.main_btn_box = ctk.CTkFrame(self.main_screen, fg_color="transparent")
        self.main_btn_box.pack(fill="x", pady=(5, 0))

        self.delete_btn = ctk.CTkButton(self.main_btn_box, text="Удалить", command=self.delete_game, fg_color="#8b0000", hover_color="#a10000", width=80, height=45)
        self.delete_btn.pack(side="left", padx=(0, 5))

        self.edit_btn = ctk.CTkButton(self.main_btn_box, text="ИЗМЕНИТЬ", command=self.open_edit_existing, fg_color="#444", hover_color="#555", width=100, height=45)
        self.edit_btn.pack(side="left", padx=5)

        self.run_btn = ctk.CTkButton(self.main_btn_box, text="ЗАПУСТИТЬ ИГРУ", command=self.run_game, fg_color="#008cff", hover_color="#0070cc", font=("Arial", 14, "bold"), height=45)
    ...
    def open_edit_existing(self):
        ...
        self.exe_entry.insert(0, game["exe_path"])
        ...
        for opt in self.runner_options:
            if game["runner_type"] == "proton" and game["proton_path"] in opt:
    ...
    def init_add_screen(self):
        ctk.CTkLabel(self.add_screen, text="Настройки игры", font=("Arial", 16, "bold")).pack(pady=(0, 5))
        self.exe_entry = ctk.CTkEntry(self.add_screen, placeholder_text="Путь к exe...")
        self.exe_entry.pack(fill="x", pady=2)
        ctk.CTkButton(self.add_screen, text="Обзор .exe", height=25, command=self.choose_exe).pack(anchor="e", pady=(0, 5))
        self.name_entry = ctk.CTkEntry(self.add_screen, placeholder_text="Имя игры...")
        ...
        self.proton_entry = ctk.CTkEntry(self.proton_frame, placeholder_text="Путь к Proton...")
        self.proton_entry.pack(fill="x", side="left", expand=True, padx=(0, 5))
        self.prefix_entry = ctk.CTkEntry(self.add_screen, placeholder_text="Путь к префиксу (WINEPREFIX)...")
        ...
        ctk.CTkCheckBox(self.add_screen, text="Включить MangoHud", variable=self.mangohud_var).pack(anchor="w", padx=10, pady=2)
        self.gamescope_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(self.add_screen, text="Включить Gamescope", variable=self.gamescope_var).pack(anchor="w", padx=10, pady=2)
        self.status_label = ctk.CTkLabel(self.add_screen, text="", text_color="red")
        self.status_label.pack(pady=5)
        
        btn_box = ctk.CTkFrame(self.add_screen, fg_color="transparent")
        btn_box.pack(fill="x", side="bottom", pady=5)
        ctk.CTkButton(btn_box, text="ОТМЕНА", fg_color="#444", command=self.cancel_add).pack(side="left", expand=True, padx=(0,5))
        ctk.CTkButton(btn_box, text="СОХРАНИТЬ", fg_color="green", command=self.save_new_game).pack(side="right", expand=True, padx=(5,0))
    ...
    def choose_exe(self):
        ...
        if p:
            ...
            self.name_entry.delete(0, tk.END); self.name_entry.insert(0, os.path.splitext(os.path.basename(p))[0])
    ...
    def clear_add_inputs(self):
        for e in [self.exe_entry, self.name_entry, self.prefix_entry]: e.delete(0, tk.END)
        self.mangohud_var.set(False); self.gamescope_var.set(False)
    ...
    def save_new_game(self):
        ...
        new_data = {
            ...
            "wineprefix": self.prefix_entry.get().strip(), "dxvk_version": self.dxvk_dropdown.get(),
            "vkd3d_version": self.vkd3d_dropdown.get(), "use_mangohud": self.mangohud_var.get(),
            "use_gamescope": self.gamescope_var.get(), "launch_count": self.selected_game["launch_count"] if self.is_editing else 0
        }
    ...
    def delete_game(self):
        if self.selected_game: self.games.remove(self.selected_game); self.save_games(); self.update_list()
    ...
    def update_list(self):
        ...
        for g in self.games:
            if self.search_query.lower() in g["name"].lower():
                btn = ctk.CTkButton(self.scroll_frame, text=g['name'], anchor="w", fg_color="transparent", command=lambda game=g: self.select_game(game))
                btn.pack(fill="x", pady=2); self.game_buttons.append((g, btn))

    def filter_games(self, event): self.search_query = self.search_entry.get(); self.update_list()

    def select_game(self, game):
        self.selected_game = game
        for g, b in self.game_buttons: b.configure(fg_color="#1f538d" if g == game else "transparent")
        info = f"Раннер: {game['runner_type'].upper()}\nПрефикс: {game['wineprefix'] or 'Default'}\nТрансляторы: DXVK: {game.get('dxvk_version','None')} | VKD3D: {game.get('vkd3d_version','None')}\nОптимизации: MangoHud: {'ДА' if game.get('use_mangohud') else 'НЕТ'}\nЗапусков: {game.get('launch_count',0)}"
    ...
    def run_game(self):
        ...
        if game["runner_type"] == "proton":
            env["STEAM_COMPAT_DATA_PATH"] = game["wineprefix"] or os.path.join(os.path.dirname(game["exe_path"]), "prefix")
        ...
        game["launch_count"] += 1; self.save_games(); self.select_game(game)

    def init_settings_screen(self):
        ctk.CTkLabel(self.settings_screen, text="Центр компонентов", font=("Arial", 16)).pack(pady=10)
        self.tabview = ctk.CTkTabview(self.settings_screen, height=400)
        self.tabview.pack(fill="both", expand=True)
        
        # 🚀 Добавили кастомные настройки API для разных репозиториев (GitHub и Forgejo)
        self.tab_configs = {
        ...
        for key, conf in self.tab_configs.items():
            ...
            scroll = ctk.CTkScrollableFrame(tab, height=200); scroll.pack(fill="x", pady=5)
            ctk.CTkButton(tab, text=f"Обновить список", command=lambda c=conf, s=scroll, k=key: self.load_data_thread(c, s, k)).pack(fill="x", pady=5)
            
        ctk.CTkButton(self.settings_screen, text="НАЗАД", command=lambda: self.show_screen(self.main_screen)).pack(fill="x", side="bottom", pady=10)

    def load_data_thread(self, conf, scroll, component_type):
        def worker():
            try:
                # 🛠️ Переключаем базовый URL в зависимости от типа сервера
                if conf["is_forgejo"]:
    ...
    def start_download(self, url, tag, component_type):
        def worker():
            ...
            print(f"Начало скачивания {tag}...")
            try:
                urllib.request.urlretrieve(url, path)
                print(f"Распаковка {tag}...")
                
                # Распаковываем xz и zst через системный tar
                ...
                os.remove(path)
                print(f"✨ {tag} успешно установлен!")
            except Exception as e:
                print(f"Ошибка при скачивании или распаковке {tag}: {e}")
